## 0. 解题思路: CachePage
### 0.1 消息格式
DefaultByteMessage结构为Headers+Properties+body,
其中Headers和Properties均为封装后的<Key,Value>形式，body为字节数组的形式。

### 0.2 三种文件并发读写方法：
#### 0.2.1 MessageStoreMMap
MessageStoreMMap.putMessage()方法使用MappedByteBuffer，以小logfile(2MB)的形式记录消息，每个TOPIC或QUEUE中为每个线程建立单独的logfile，各线程写操作时无锁竞争。
MessageStoreMMap.pullMessage()方法使用MappedByteBuffer，读取小logfile内记录的消息数据，线程间的并发读取不需要加锁。同时在每个TOPIC或QUEUE内按照id递增读取，可以保证消息读取不错位置。
#### 0.2.2 MessageStoreMMapSingle
MessageStoreMMapSingle.putMessage()方法使用MappedByteBuffer，为每一个TOPIC或QUEUE创建一个大logfile(64MB)，当2个及以上线程同时写一个文件时，通过锁竞争获取写锁。
MessageStoreMMapSingle.pullMessage()方法使用内存映射，线程无锁并发读取attach的TOPIC或QUEUE，由于同一个TOPIC保存在同一个文件内，自然可以保证有序。
#### 0.2.3 MessageStoreSingle
MessageStoreSingle与MessageStoreMMapSingle方法的唯一不同之处是采用了RandomAccessFile
...
此版本为较通用版本
```text
  MsgSerialize_version_1 (Byte)
  flag: flag[4]
  header: header_num[4]| ... key_length[4]|value_length[4]|key[?]|value[?] ...
  property: property_num[4]| ... key_length[4]|value_length[4]|key[?]|value[?] ...
  body: body_length[4]| ... body[]
```
序列化过程采用jdk提供的String.getBytes()方法，并自定义int4Byte()和byte4Int()方法。
```java
...
#### 0.3.2 MsgSerializer2
此版本为特殊优化版本
```text
  MsgSerialize2_version2 (Byte) if sumLength < 127
  sumLength[4]
  body: body_length[1]|body[body_length]
  header: headers_nums[1]|...key_length[1]|key[key_length]|value_length[1]|value[value_length]...
  property: same as header
  etc..
```
### 0.4 成绩
> pullNum:40000000
